-- 048_team_assets_session_date_order.sql
-- Keep the Assets page ordered by the session date first, newest to oldest.

create index if not exists sessions_asset_page_date_idx
  on public.sessions (session_date desc, dock_out_at desc, id);

drop function if exists public.get_team_asset_page(
  uuid,
  public.asset_type,
  uuid,
  integer,
...
as $$
  ...
  order by
    ...
    filtered.dock_out_at desc nulls last,
