-- Read computed gear alert state for specific gear items and normalize item-specific thresholds.

alter type public.notification_event_type add value if not exists 'gear_warning';
alter type public.notification_event_type add value if not exists 'gear_critical';

with critical_only_metrics as (
  select
    gear_item_id,
    metric,
    min(threshold_value) as threshold_value
  from public.gear_alert_rules
  group by gear_item_id, metric
  having
    bool_or(severity = 'critical'::public.gear_alert_severity)
    and not bool_or(severity = 'warning'::public.gear_alert_severity)
),
promoted_rules as (
  select distinct on (gar.gear_item_id, gar.metric)
    gar.id
  from public.gear_alert_rules gar
  join critical_only_metrics critical_only
    on critical_only.gear_item_id = gar.gear_item_id
   and critical_only.metric = gar.metric
   and critical_only.threshold_value = gar.threshold_value
  where gar.severity = 'critical'::public.gear_alert_severity
  order by gar.gear_item_id, gar.metric, gar.created_at, gar.id
)
update public.gear_alert_rules gar
set severity = 'warning'::public.gear_alert_severity
from promoted_rules
where gar.id = promoted_rules.id;
...
begin
  if exists (
    ...
    where table_schema = 'public'
      and table_name = 'gear_alert_rules'
      and column_name = 'is_refurbished_rule'
  ) then
    update public.gear_alert_rules
    set is_refurbished_rule = false
    where is_refurbished_rule;
  end if;
end $$;

update public.notifications
set message = case
  when event_type::text = 'gear_critical' then
    case
      when message ~ ' is in critical mode\. Review the Gear thresholds\.$' then
        regexp_replace(
          message,
          ' is in critical mode\. Review the Gear thresholds\.$',
          ' is approaching its usage limit.'
        )
      else 'Gear item is approaching its usage limit.'
    end
  when event_type::text = 'gear_warning' then
    case
      when message ~ ' is in warning mode\. Review the Gear thresholds\.$' then
        regexp_replace(
          message,
          ' is in warning mode\. Review the Gear thresholds\.$',
          ' is past due and needs review.'
        )
      else 'Gear item is past due and needs review.'
    end
  else message
end
where event_type::text in ('gear_critical', 'gear_warning');

create or replace function public.get_team_gear_alert_rows(
  p_team_id uuid,
  p_gear_item_ids uuid[] default null
)
returns table (
  gear_item_id uuid,
  team_id uuid,
...
as $$
  with base as (
    ...
    where gi.team_id = p_team_id
      ...
      and (p_gear_item_ids is null or gi.id = any(p_gear_item_ids))
  ),
  linked_sessions as (
    select
      sgu.gear_item_id,
...
as $$
  ...
  alert_rows as (
    select *
    from public.get_team_gear_alert_rows(
      p_team_id,
      coalesce(
        (select array_agg(filtered_base.gear_item_id) from filtered_base),
        array[]::uuid[]
      )
    )
  ),
  computed as (
    select
      filtered_base.gear_item_id,
      filtered_base.team_id,
      filtered_base.name,
      filtered_base.gear_type,
      filtered_base.serial_number,
      filtered_base.barcode,
      filtered_base.status,
      filtered_base.condition,
      coalesce(alert_rows.usage_count, 0) as usage_count,
      coalesce(alert_rows.usage_minutes, 0) as usage_minutes,
      coalesce(alert_rows.alert_state, 'none'::public.gear_alert_state) as alert_state,
      coalesce(alert_rows.triggered_alert_count, 0)::bigint as triggered_alert_count,
      filtered_base.created_at
    from filtered_base
    left join alert_rows
      on alert_rows.gear_item_id = filtered_base.gear_item_id
  ),
  filtered as (
    select *
    from computed
...
alter table public.gear_alert_rules
drop column if exists is_refurbished_rule;
