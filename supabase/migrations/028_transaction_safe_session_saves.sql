-- 028_transaction_safe_session_saves.sql
-- Transaction-safe save operations for session setup and gear links.
...
create or replace function public.replace_session_gear_usage_atomic(
  p_session_id uuid,
  p_team_id uuid,
  p_gear_item_ids uuid[],
  p_linked_by_profile_id uuid
)
returns void
language plpgsql
set search_path = public
...
declare
  session_team_id uuid;
  gear_item_ids uuid[] := coalesce(p_gear_item_ids, array[]::uuid[]);
  distinct_gear_item_count integer;
begin
  session_team_id := public.team_id_for_session(p_session_id);
  ...
  select count(distinct gear_item_id)
  into distinct_gear_item_count
  from unnest(gear_item_ids) as gear(gear_item_id);

  if distinct_gear_item_count <> cardinality(gear_item_ids)
     or exists (
      select 1
      from unnest(gear_item_ids) as gear(gear_item_id)
      left join public.gear_items gi
        on gi.id = gear.gear_item_id
       and gi.team_id = p_team_id
      where gi.id is null
    ) then
    raise exception 'Invalid gear item scope';
  end if;

  delete from public.session_gear_usage sgu
  where sgu.session_id = p_session_id;

  insert into public.session_gear_usage (
    session_id,
    gear_item_id,
