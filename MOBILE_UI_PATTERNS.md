...
This document is the mobile UI standard for operational screens, list pages,
mobile card lists, and edit surfaces in Sailog.
...
- Mobile header shows:
  ...
  - back arrow goes to its parent. (ex. Camp ID back to venue parent)
...
- Mobile: cards.
- Desktop: table.
- This section is the source of truth for mobile card-list behavior. Desktop
  table behavior lives in `DESKTOP_UI_PATTERNS.md`.
...
- Keep existing cards visible while more data loads.
...
- Card tap target should cover the full card and navigate to the detail page.
...
- Use overflow menus for destructive or secondary row/card actions when space is
  tight.
...
- Save buttons inside mobile Drawers follow the Save CTA standard in this
  document: fixed footer, `h-11 w-full`, disabled while pending, spinner plus
  pending copy.
