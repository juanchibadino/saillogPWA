...
import type { UsersChromeData } from "@/features/users/data"
import {
...
function UsersMobileFilterDrawer({
  disabled,
  isNavigating,
  onNavigate,
  options,
  ...
  const [draftTeamId, setDraftTeamId] = useState(selectedValue)
  const hasActiveFilter = selectedValue.length > 0
  const selectedDraftOption = options.find((option) => option.value === draftTeamId)
  const clearOption = options.find((option) => option.value === "")
      ...
      <DrawerContent className="h-[85dvh] overflow-hidden data-[vaul-drawer-direction=bottom]:max-h-[85dvh]">
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 pb-6">
          <div className="space-y-2">
            <label
              htmlFor="mobile-users-team-filter"
              className="text-xs font-medium uppercase tracking-wide text-muted-foreground"
            >
              Team
            </label>
            <select
              id="mobile-users-team-filter"
              name={USERS_TEAM_FILTER_QUERY_KEY}
              value={draftTeamId}
...
export function UsersRouteShell({
  canManageUsers,
  children,
  chromeData,
  currentPage,
  ...
  const teamFilterOptions: UsersFilterOption[] = [
    {
      value: "",
      label: "All",
      href: buildUsersTeamFilterHref({
        pathname,
        search: searchParams.toString(),
        nextTeamId: null,
      }),
    },
    ...chromeData.teamOptions.map((team) => ({
      value: team.id,
      label: team.name,
      href: buildUsersTeamFilterHref({
        pathname,
        ...
        {isFilterNavigationBusy ? (
          <>
            <div className="fixed inset-x-0 bottom-[var(--mobile-bottom-nav-total-height)] top-[var(--mobile-header-total-height)] z-30 flex items-center justify-center bg-background/20 md:hidden">
              <div
                role="status"
                aria-label="Loading filtered members"
