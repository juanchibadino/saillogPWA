...
export function hasBillingPeriodEndedCore(input) {
  const periodEndTimestamp = toFiniteTimestamp(input.currentPeriodEndAt)
  const nowTimestamp = toFiniteTimestamp(input.now)

  return (
    periodEndTimestamp !== null &&
    nowTimestamp !== null &&
    periodEndTimestamp <= nowTimestamp
  )
}

export function shouldDowngradePaidPlanToFreeCore(input) {
  const normalizedPlanTier = normalizePlanTierCore(input.planTier)

  if (!PAID_PLAN_TIERS_CORE.includes(normalizedPlanTier)) {
    return false
  }
  ...
  return hasBillingPeriodEndedCore({
